    ...
    def move_to_target(self):
        ...
        if self.move_x:
            ...
            if self.rotation_times<16:  
                velocity.angular.z = -1.57 if self.target_y < 0 else 1.57 
                self.pub_vel.publish(velocity)
            ...
            if abs(moving_position_y)>0.05:
                velocity.linear.x=0.5  
                self.pub_vel.publish(velocity)
                print(f"the position is {0,self.current_y}")
                self.current_y += (velocity.linear.x * 0.1) if self.target_y > self.current_y else (-velocity.linear.x * 0.1)
